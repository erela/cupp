...
def read_peofile(filename):
	...
	if os.path.isfile(filename):
		...
		G_PROFILE["prefixes"] = profile_reader.get("global","prefixes").split(",")
		G_PROFILE["suffixes"] = profile_reader.get("global","suffixes").split(",")
...
def generate_wordlist_from_profile():
    ...
    # Add prefixes
    basic_with_prefixes = [];
    for pref in G_PROFILE["prefixes"]:
    	for curr in basic_words:
    		basic_with_prefixes.extend([pref + curr,
    			                        pref.title()+curr,
    			                        pref.upper()+curr,
    			                        pref.lower()+curr])
    ...
    if CONFIG["global"]["special_chars_pading"] == "yes":
        chars = CONFIG["global"]["chars"]
        for c_word in basic_with_prefixes:
    	    for ch in chars:
    		    ab.extend([c_word,str(c_word + ch)])
        uniqueListFinished.extend(remove_duplicates_from_list(ab))

    # Add suffixes
    basic_with_suffixes = [];
    for suff in G_PROFILE["suffixes"]:
    	for curr in basic_words:
    		basic_with_suffixes.extend([curr+suff,
    			                        curr+suff.title(),
    			                        curr+suff.upper(),
    			                        curr+suff.lower()])
    ...
    if CONFIG["global"]["special_chars_pading"] == "yes":
        chars = CONFIG["global"]["chars"]
        for c_word in basic_with_suffixes:
    	    for ch in chars:
    		    ab.extend([c_word,str(c_word + ch)])
        uniqueListFinished.extend(remove_duplicates_from_list(ab))

    uniqueListFinished.extend(remove_duplicates_from_list(basic_with_prefixes))
    uniqueListFinished.extend(remove_duplicates_from_list(basic_with_suffixes))
